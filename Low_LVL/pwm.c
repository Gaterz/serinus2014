...
void controlMotor1_invert(int dutyCycle)
{
	if(dutyCycle<0)
	{
		...
		in_Mot1(1,0); 					// Commande du pont en H
		dutyCycle = -dutyCycle;			// Passage du rapport cyclique en positif
		pwmU1var((unsigned int)(10000 - dutyCycle)); // Gיnיration de la PWM
	}
	else if (dutyCycle>0)
	{
	    //Avance
  ...
  else
  {
    ...
    pwmU1var(0); // Gיnיration de la PWM
  }
}
...
void controlMotor2_invert(int dutyCycle)
{
	if(dutyCycle<0)
	{
		...
		in_Mot2(1,0); 					// Commande du pont en H
		dutyCycle = -dutyCycle;			// Passage du rapport cyclique en positif
		pwmU2var((unsigned int)(10000 - dutyCycle)); // Gיnיration de la PWM
	}
	else if(dutyCycle>0)
	{
	    //Avance
  ...
  else
  {
    ...
    pwmU2var(0); // Gיnיration de la PWM
  }
}
...
void initPWM()
{
    ...
    */
    ...
    PWM_MR0 = 10000 ;        //Pיriode de PWMTC de 10 000 * 17 ns = 170 us, soit Fpwm = 5900 Hz
